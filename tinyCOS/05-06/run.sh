...
# Build the kernel
$CC $CFLAGS -Wl,-Tkernel.ld -Wl,-Map=kernel.map -o kernel.elf start.c kernel.c common.c
...
$QEMU -machine virt -m 128M -bios default -nographic -serial mon:stdio --no-reboot -kernel kernel.elf